...
def detect_and_crop_from_video(video_path):
    STRIDES, ANCHORS, NUM_CLASS, XYSCALE = utils.load_config(FLAGS)
    input_size = FLAGS.size
    video_path = FLAGS.video
    # get video name by using split method
    video_name_with_extension = video_path.split('/')[-1]
    ...
    while True:
        ...
        if frame_num % crop_rate == 0:
            ...
            try:
                ...
                pass          
            crop_objects(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB), pred_bbox, final_path, allowed_classes, video_name)
        else:
            pass
...
def main(argv):
    detect_and_crop_from_video(video_path) #to be added
